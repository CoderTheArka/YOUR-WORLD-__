...
function getData1() {firebase.database().ref('users/').on('value', function(snapshot) {document.getElementById("gg1").innerHTML = "";snapshot.forEach(function(childSnapshot) {childKey  = childSnapshot.key;
  Room_names = childKey;
  
 //Start code
 console.log("USERS - "+ Room_names);
 localStorage.setItem('Names', Room_names);
 row = "<div class='dfg' id='"+Room_names+"'><div id='bhbh' class='image'><img id='srcgg'></div><div id='"+Room_names+"1"+"' class='text' ><p id='emailofother'>"+Room_names+"</p><p id='emailofother1' class='ff'></p></div></div><hr id='"+Room_names+"2"+"'>";

 document.getElementById("gg1").innerHTML += row;
//End code
 });});}
getData1();
